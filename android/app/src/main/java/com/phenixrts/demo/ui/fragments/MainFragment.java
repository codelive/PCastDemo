...
import static com.phenixrts.demo.Constants.STOP_PUBLISHER;
import static com.phenixrts.demo.Constants.STREAM_ID;
import static com.phenixrts.demo.Constants.STREAM_LIST_LENGTH;
import static com.phenixrts.demo.Constants.VIEW_STREAM;
import static com.phenixrts.demo.StatusQualityValues.ALL_DOWNLOAD;
import static com.phenixrts.demo.StatusQualityValues.ALL_UPLOAD;
import static com.phenixrts.demo.StatusQualityValues.AUDIO_ONLY_LIMITED;
import static com.phenixrts.demo.StatusQualityValues.AUDIO_ONLY_NONE;
...
public final class MainFragment extends BaseFragment implements View.OnClickListener, IMainView, StreamIdAdapter.OnItemClickListener, CompoundButton.OnCheckedChangeListener,
  ...
  protected void bindEventHandlers(View view) {
    ...
    LayoutUtil.animateView(this.buttonStop);
    if (!this.isError) {
  ...
  public void onResume() {
    ...
    if (this.phenixApplication.isStopPublish()
      ...
      LayoutUtil.setLayout(this.activity,
              true,
              this.buttonAudio,
              this.buttonVideo,
  ...
  public void onItemClick(View itemView, int position) {
    ...
    LayoutUtil.animateView(this.imageLoad);
  ...
  public void onConfigurationChanged(Configuration newConfig) {
    ...
    this.arcImage.getLayoutParams().width = getResources().getDimensionPixelSize(
        R.dimen.with_parabol);
  ...
  private void layoutPortrait(boolean tabletSize) {
    ...
    if (tabletSize) {
      if (this.isFullScreen) {
        ...
        LayoutUtil.setLayoutTablet(this.viewList, true);
        LayoutUtil.setPreviewDimensionsTablet(this.activity, getViewLocal());
      }
    } else {
      if (this.isFullScreen) {
        this.onFullScreen();
      } else {
        LayoutUtil.setPreviewDimensions(this.activity, getViewLocal());
      }
    }
    this.viewAnimation.setPadding(LayoutUtil.dpToPx(this.activity, 4),
            LayoutUtil.dpToPx(this.activity, 4),
            LayoutUtil.dpToPx(this.activity, 4),
            LayoutUtil.dpToPx(this.activity, 4));
  }
  ...
  private void layoutLandscape(boolean tabletSize) {
    ...
    this.viewAnimation.setPadding(LayoutUtil.dpToPx(this.activity, 4),
            LayoutUtil.dpToPx(this.activity, 0),
            LayoutUtil.dpToPx(this.activity, 4),
            LayoutUtil.dpToPx(this.activity, 4));
  }
  ...
  private void onChangeLayout() {
    if (this.viewLocal != null) {
      this.viewLocal.setContentPadding(LayoutUtil.dpToPx(this.activity, 2),
              LayoutUtil.dpToPx(this.activity, 2),
              LayoutUtil.dpToPx(this.activity, 2),
              LayoutUtil.dpToPx(this.activity, 2));
      this.viewLocal.setRadius(LayoutUtil.dpToPx(this.activity, 6));
    }
    boolean tabletSize = getResources().getBoolean(R.bool.isTablet);
    if (tabletSize) {
      LayoutUtil.setPreviewDimensionsTablet(this.activity, getViewLocal());
  ...
  private void onEventStopStreamDetail() {
    if (this.activity != null && isVisible()) {
      showDialog("Notification", getString(
          R.string.stop_video_pub, ""), new DialogUtil.ActionDialog() {
        @Override
        public AppCompatActivity getContext() {
          return (AppCompatActivity) activity;
        }
